...
export class NewsService {
  ...
  updateNews(
    ...
    if (typeof image === 'object') {
      ...
      newsData = {
        ...
        content: content,
        imagePath: image,
        creator: null // handle this logic in server for security
      };
    }

    this.http
      .put('http://localhost:3000/api/news/' + id, newsData)
      .subscribe(response => {
  ...
  deletePost(newsId: string) {
    this.http
      .delete('http://localhost:3000/api/news/' + newsId)
      .subscribe(() => {
        const updatedNews = this.news.filter(mynews => mynews.id !== newsId);
        this.news = updatedNews;
        this.newsUpdated.next([...this.news]);
      });
  }
